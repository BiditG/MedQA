...
function parseCSV(text: string): MCQ[] {
  ...
  // Detect if the CSV has no header row. If the first row doesn't contain
  // any expected column names (like 'question' or 'option'), assume the
  // file is header-less and apply the known column order used by
  // `pastquestions.csv`.
  const headerLower = header.join(' ').toLowerCase()
  const looksLikeHeader =
    /\b(question|optiona|optionb|optionc|optiond|answer|explanation|subject|topic|id)\b/.test(
      headerLower,
    )
  if (!looksLikeHeader) {
    header = [
...
export default function CeePastPracticePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch('/data/topic-mapping.json')
        .then((r) => (r.ok ? r.json() : {}))
        .catch(() => ({})),
      fetch('/data/pastquestions.csv').then((r) => {
  ...
  if (loading) return <div className="p-4">Loading past questions…</div>
        ...
        <div className="mx-auto max-w-4xl">
          {!started && (
            <div className="rounded-2xl border p-4">
              <h2 className="mb-2 text-lg font-semibold">Past Questions</h2>
              <p className="mb-4 text-sm text-muted-foreground">
                Choose subject, topic and number of questions to practice from
                the past question bank. Casual mode gives immediate feedback;
                Exam mode is timed.
              </p>
